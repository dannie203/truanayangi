...
const out=mkdtempSync(join(tmpdir(),'pool-test-'));try{
 ...
 const {emptyProfile,validateProfile,personalFoods,personalSelector,encodePoolShare,decodePoolShare}=require(join(out,'personal-pool.js'));const {foods}=require(join(out,'foods.js'));
 ...
 const shareCode=encodePoolShare(p);const decoded=decodePoolShare(shareCode);assert.equal(decoded.custom.length,1);assert.equal(decoded.custom[0].name,'Solo');assert.equal(decoded.custom[0].price,85);assert.equal(decoded.custom[0].veg,true);assert.deepEqual(decoded.disabled,all);assert.equal(decodePoolShare('invalid'),null);assert.equal(decodePoolShare(''),null);
 console.log('PASS: empty, single-item, removed IDs, price boundaries, feasible mean, personalized selection and pool sharing.');
